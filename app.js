class GitHubDashboard {
    constructor(options = {}) {
        ...
        const defaultTokenParts = ['ghp', '_UWBE356g70UQ9hXbVf', 'MlLOf1C4CnXc1QFfGb'];
    ...
    updateProfileInfo(usersData) {
        ...
        if (followInfoEl) {
            const followParts = usersData.map(user => `${user.followers || 0} followers for @${user.login}`);
            followInfoEl.textContent = followParts.join(' • ');
        }
    ...
    showProfileError(message) {
        ...
        if (followInfoEl) followInfoEl.textContent = 'Followers unavailable';
